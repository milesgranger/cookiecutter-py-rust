...
if [[ $TRAVIS_OS_NAME == "osx" ]]; then
    ...
    pip install -v {{ cookiecutter.package_name }} --no-index -f ./wheelhouse/
    ...
    for whl in /io/wheelhouse/{{ cookiecutter.package_name }}*.whl; do
    ...
    for PYBIN in /opt/python/cp{35,36}*/bin/; do
        pushd /io
        "${PYBIN}/pip" uninstall {{ cookiecutter.package_name }}
        "${PYBIN}/pip" install {{ cookiecutter.package_name }} --no-index -f /io/wheelhouse
        popd
